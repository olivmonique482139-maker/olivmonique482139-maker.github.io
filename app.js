import { renderRoute, recipes as recipeLibrary } from '/pages.js?v=6';
...
function lockBody() {
  ...
  $$('video[data-ambient-video], #heroVideo, #ipCompanionVideo').forEach(video => {
    if (!video.paused) {
...
const stepTemplates = {
  0: () => `
    <section class="quiz-step" aria-labelledby="quizTitle">
      <div class="mascot-stage welcome-stage"><img class="mascot-img mascot-welcome" src="/assets/ip/cat-welcome-2d-v01.webp" alt="2D Q 版布丁抬起猫爪向你打招呼" decoding="async"><span class="mascot-speech">我是布丁，先认识一下它吧。</span></div>
...
function showGenerating() {
  ...
  quizContent.innerHTML = `<section class="generating" aria-labelledby="quizTitle"><img class="mascot-img mascot-thinking" src="/assets/ip/cat-thinking-720.png" alt="" aria-hidden="true" decoding="async"><p class="step-kicker">规则版本 DEMO-2026.08</p><h1 id="quizTitle">正在为${safeName()}配餐…</h1><div class="generation-list"><span>估算每日份量</span><span>检查蛋白偏好与风险提示</span><span>平衡首次尝试的口味</span></div></section>`;
...
function resultTemplate() {
  ...
  return `<section class="result-step" aria-labelledby="quizTitle">
    <div class="result-head"><img class="mascot-img mascot-success" src="/assets/ip/cat-success-bowl-720.png" alt="2D Q 版布丁抱着猫碗庆祝推荐完成" decoding="async"><p class="step-kicker">专属试吃组合 · 可继续调整</p><h1 id="quizTitle">${safeName()}的开饭计划</h1><p>根据 ${escapeHtml(state.weight)} kg 体重、口味偏好与“${goalLabels[state.goal] || '日常状态'}”目标生成的演示组合。</p><div class="plan-reason"><span>${dietLabels[state.diets[0]] || '已记录当前饮食'}</span><span>${state.picky === 'very' ? '熟悉口味优先' : '保留口味多样性'}</span><span>预计约吃 ${days} 天</span><span>非医疗建议</span></div></div>
...
function renderCart() {
  if (!cart) {
    cartContent.innerHTML = `<div class="cart-empty"><img class="mascot-img mascot-empty" src="/assets/ip/cat-empty-bowl-sad-720.png" alt="2D Q 版布丁抱着空猫碗" decoding="async"><h3>购物袋还是空的</h3><p>先认识一下你的猫咪，我们会为它搭好第一箱。</p><button class="button button-dark" data-cart-quiz>开始定制 →</button></div>`;
    return;
  }
  cart.checkout ||= {};
...
const ipCompanion = $('#ipCompanion');
const ipCompanionVideo = $('#ipCompanionVideo');
if (ipCompanion && ipCompanionVideo) {
  const companionShouldStayStill = routeName !== 'home' || matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (companionShouldStayStill) {
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden && !ipCompanionVideo.paused) {
        ...
        ipCompanionVideo.pause();
      } else if (!document.hidden && ipCompanionVideo.dataset.autoPaused === 'true' && !$$('dialog').some(dialog => dialog.open)) {
        delete ipCompanionVideo.dataset.autoPaused;
        ipCompanionVideo.play().catch(() => {});
      }
    });
  }
  ipCompanionVideo.addEventListener('error', () => ipCompanion.classList.add('is-static'));
}
